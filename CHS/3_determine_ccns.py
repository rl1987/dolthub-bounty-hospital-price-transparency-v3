...
def main():
    ...
    for in_row in csv_reader:
        ...
        phone_number = in_row.get('phone').replace(')', '').replace(' ', '').replace('-', '').replace(')', '')

        if phone_number != '':
            res = db.sql('SELECT `cms_certification_num` FROM `hospitals` WHERE `phone_number` = "{}";'.format(phone_number), 
                    result_format='json')
